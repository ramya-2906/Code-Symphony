...
class Solution:
    def minOperations(self, nums: List[int]) -> int:
        ...
        # Preprocess: build segment tree
        seg = SegmentTree(nums)

        # Preprocess: build value -> sorted list of indices
        value_indices = {}
        ...
        def recurMinOperations(low, high):
            if low > high:
                ...
                # If the number is already 0 then no action needed
                # Actually this never happens but just to be safe lol
                return int(nums[low] != 0)

            # Get minimum number in range
            ...
            # Indices of min nummber within low and high
